> **Lifecycle status (2026-07-21 spec IA reconciliation):** proposal only — nothing under `openspec/` is current runtime authority (per `AGENTS.md`/`CLAUDE.md`; the runtime authority is `specs/domains/platform-foundation/features/runtime-topology/technical.md`). NOT IMPLEMENTED as proposed. The overlapping-execution-paths problem it describes was resolved differently: the copilot agent/action pipeline over `internal/jobs` is the single realized path. Tasks below unexecuted as written.
