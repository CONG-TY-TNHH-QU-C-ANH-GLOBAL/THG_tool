...
const REAL = ['../content/facebook/commenting/comment_composer', '../content/facebook/commenting/comment_constants', '../content/proof', '../content/navreport'];
  ...
  if (typeof env.loadCommentingOutbound === 'function') {
    ...
    //      executeComment phase-helper extraction (happy path + editor-drift abort). ----
    {
      ...
      // (a) success path
      {
        ...
        const { api: a2, restore: r2 } = env.loadCommentingOutbound({ realModules: ['../content/facebook/commenting/comment_constants', '../content/proof', '../content/navreport'], singletons: stubs, document: dom.doc });
        try {
      ...
      { // (b) gate-3 editor drift → context_drift (editor's article belongs to a DIFFERENT post)
        ...
        stubs.THGCommentComposer.findComposerEntry = () => ({ el: dom.editor, reason: 'ok', candidates: [] });
        const { api: a3, restore: r3 } = env.loadCommentingOutbound({ realModules: ['../content/facebook/commenting/comment_constants', '../content/proof', '../content/navreport'], singletons: stubs, document: dom.doc });
        try {
      ...
      { // (c) executeCommentInFeed success path (findCommentEditor resolves the editor in feed)
        const dom = makeDom(ID);
        stubs.THGCommentComposer.findComposerEntry = () => ({ el: dom.editor, reason: 'ok', candidates: [] });
        const { api: a4, restore: r4 } = env.loadCommentingOutbound({ realModules: ['../content/facebook/commenting/comment_constants', '../content/proof', '../content/navreport'], singletons: stubs, document: dom.doc });
        try {
      ...
      { // (d) executeCommentInFeed with no post id → early comment_target_not_post_permalink
        const { api: a5, restore: r5 } = env.loadCommentingOutbound({ realModules: ['../content/facebook/commenting/comment_constants', '../content/proof', '../content/navreport'], singletons: stubs });
        try {
