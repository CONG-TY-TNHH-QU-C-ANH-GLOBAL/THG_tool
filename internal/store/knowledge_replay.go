...
// knowledge_events table goes through Record* in knowledge_events.go,
// not here.
//
...
// KnowledgeReplayEvent is one row of the replay timeline. Carries
// enough context for the UI to render a card without follow-up
// queries — the assembled trace + budget + outcome are all on one row.
//
...
type KnowledgeReplayEvent struct {
...
// ListKnowledgeReplayEventsForOrg returns recent retrieval events
// joined with their outcome events. Newest-first. `before` is a
// pagination cursor — pass the smallest OccurredAt from the previous
// page (empty string fetches page 1). `limit` is capped at 100.
//
...
func (s *Store) ListKnowledgeReplayEventsForOrg(ctx context.Context, orgID int64, before string, limit int) ([]KnowledgeReplayEvent, error) {
	if orgID <= 0 {
		return nil, fmt.Errorf("knowledge_replay: org_id required")
	}
	...
	defer rows.Close()
	out := make([]KnowledgeReplayEvent, 0, limit)
	for rows.Next() {
		var ev KnowledgeReplayEvent
		var blob string
...
// GetKnowledgeReplayEvent returns one retrieval event by ID, including
// its outcome (if any). Returns sql.ErrNoRows if the retrieval does
// not exist OR belongs to another org.
func (s *Store) GetKnowledgeReplayEvent(ctx context.Context, orgID int64, retrievalID string) (*KnowledgeReplayEvent, error) {
...
// KnowledgeStats is the aggregate-stats payload for the Replay
// dashboard headline. Computed on demand — cheap enough at MVP scale.
type KnowledgeStats struct {
	...
	TopRetrieved      []KnowledgeStatsRow `json:"top_retrieved"`
	LowConversionTop  []KnowledgeStatsRow `json:"low_conversion_top"`
}

// KnowledgeStatsRow is one entry of the top-retrieved /
// low-conversion lists.
type KnowledgeStatsRow struct {
...
// GetKnowledgeStatsForOrg computes the headline metrics for the
// Replay dashboard. Cheap aggregate queries; no caching layer needed
// at MVP volume.
func (s *Store) GetKnowledgeStatsForOrg(ctx context.Context, orgID int64) (*KnowledgeStats, error) {
	if orgID <= 0 {
		return nil, fmt.Errorf("knowledge_replay: org_id required")
	}
	out := &KnowledgeStats{}
	...
	stale, err := s.CountStaleKnowledgeAssetsForOrg(ctx, orgID, 30)
	if err == nil {
...
func (s *Store) queryStatsRows(ctx context.Context, orgID int64, query string) []KnowledgeStatsRow {
	rows, err := s.QueryContext(ctx, query, orgID)
	if err != nil {
		return nil
	}
	defer rows.Close()
	out := []KnowledgeStatsRow{}
	for rows.Next() {
		var r KnowledgeStatsRow
...
// produced by RecordKnowledgeRetrievalWithTrace — keep the two in
// sync. Pre-trace events (legacy RecordKnowledgeRetrieval) have a
// different shape; we surface what we can and leave the trace empty
// rather than failing.
