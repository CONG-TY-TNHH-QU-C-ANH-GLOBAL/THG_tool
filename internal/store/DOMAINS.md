...
Tasks, KPI, media, pricing, careers, stats + the app_tasks/task_leads connector-pipeline cluster. **Completed 2026-07-05 (AppStore dissolution PR6)**: `app_store.go` is deleted — `AppTask`/`TaskLead`/`LeadCounts` CRUD moved to `internal/store/app/tasks.go` + `task_leads.go` (reached via `s.App()`), and the idempotent app/browser-infra bootstrap moved to `app.Migrate(db)`, called by `store.New` on both dialect paths right after `sessions.Migrate`. The other former `*AppStore` files were retired earlier: sessions cluster → `internal/store/sessions/` (PR1–PR5); `identities.go`/`learning.go` deleted as proven-dead (PR5).
