...
const scriptSelect = `SELECT id, org_id, reel_id, version, content, approved, created_at FROM reel_scripts`
...
// must be the caller-computed next version number (UNIQUE(reel_id, version)
// rejects a duplicate).
func (s *Store) CreateScript(ctx context.Context, orgID, reelID int64, version int, content string) (int64, error) {
	...
	return s.insertReturningID(ctx,
		`INSERT INTO reel_scripts (org_id, reel_id, version, content) VALUES (?, ?, ?, ?) RETURNING id`,
		orgID, reelID, version, content,
	)
}
...
func (s *Store) GetLatestScript(ctx context.Context, orgID, reelID int64) (*Script, error) {
	...
	row := s.queryRowContext(ctx,
		scriptSelect+` WHERE reel_id = ? AND org_id = ? ORDER BY version DESC LIMIT 1`,
		reelID, orgID,
	)
...
func (s *Store) ListScripts(ctx context.Context, orgID, reelID int64) ([]*Script, error) {
	...
	rows, err := s.queryContext(ctx,
		scriptSelect+` WHERE reel_id = ? AND org_id = ? ORDER BY version ASC`,
		reelID, orgID,
	)
	if err != nil {
		return nil, err
...
func (s *Store) ApproveScript(ctx context.Context, orgID, scriptID int64) error {
	...
	_, err := s.execContext(ctx,
		`UPDATE reel_scripts SET approved = 1 WHERE id = ? AND org_id = ?`,
		scriptID, orgID,
	)
	return err
}
