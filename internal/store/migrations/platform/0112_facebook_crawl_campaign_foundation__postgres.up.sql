...
-- Composite tenant anchor for accounts: the campaign account pool references
-- accounts by (org_id, id), which requires a non-partial unique key on that
-- pair. accounts has only a single-column PK today. IF NOT EXISTS keeps this a
-- no-op where a later baseline already added it.
CREATE UNIQUE INDEX IF NOT EXISTS uq_accounts_org_id_id
