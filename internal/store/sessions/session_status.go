...
func (s *Store) RecordLocalSession(ctx context.Context, accountID, orgID int64, status LocalSessionStatus, errMsg string) error {
	now := time.Now().UTC()
	return s.UpsertSession(ctx, BrowserSession{
		AccountID:    accountID,
		OrgID:        orgID,
