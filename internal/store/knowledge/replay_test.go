...
func seedRetrievalWithTrace(t *testing.T, db *knowledge.Store, orgID int64, retrievalID, query string, hits []retrieval.ScoredHit) {
	t.Helper()
	...
	db.RecordRetrievalWithTrace(context.Background(), orgID, retrievalID, query, "comment_drafted", trace, budget)
}
...
func TestListKnowledgeReplayEvents_HappyPath(t *testing.T) {
	db := newKnowledgeStore(t, "replay.db")
	...
	db.RecordOutcome(ctx, 7, "ret_a", "sent")

	events, err := db.ListReplayEventsForOrg(ctx, 7, "", 10)
	if err != nil {
		t.Fatalf("List: %v", err)
	}
	if len(events) != 1 {
	...
	// Trace must round-trip â€” re-parse and check selected hit count.
...
func TestListKnowledgeReplayEvents_TenantScope(t *testing.T) {
	...
	db.RecordOutcome(ctx, 1, "ret_org1", "sent")
	db.RecordOutcome(ctx, 2, "ret_org2", "rejected")

	e1, _ := db.ListReplayEventsForOrg(ctx, 1, "", 10)
	...
	e2, _ := db.ListReplayEventsForOrg(ctx, 2, "", 10)
...
// This is the queuedâ†’sent progression the runtime emits.
func TestGetKnowledgeReplayEvent_LatestOutcomeWins(t *testing.T) {
	db := newKnowledgeStore(t, "replay.db")
	ctx := context.Background()
	seedRetrievalWithTrace(t, db, 7, "ret_x", "q", nil)
	db.RecordOutcome(ctx, 7, "ret_x", "queued")
	db.RecordOutcome(ctx, 7, "ret_x", "sent")

	ev, err := db.GetReplayEvent(ctx, 7, "ret_x")
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	if ev.Outcome != "sent" {
...
func TestGetKnowledgeStatsForOrg(t *testing.T) {
	...
	stats, err := db.GetStatsForOrg(ctx, 7)
	if err != nil {
