...
func mustSetupSource(t *testing.T, db *Store, orgID int64) int64 {
	t.Helper()
	src, err := db.UpsertKnowledgeSource(context.Background(),
...
// Happy-path round trip — proves schema + scanner agree on column order.
func TestUpsertKnowledgeAsset_RoundTrip(t *testing.T) {
	...
	saved, err := db.UpsertKnowledgeAsset(ctx, a)
	if err != nil {
...
func TestGetKnowledgeAsset_ForeignOrgIsNotFound(t *testing.T) {
	...
	sid := mustSetupSource(t, db, 1)
	saved, err := db.UpsertKnowledgeAsset(ctx, newTestAsset(1, sid, "ext_1", "Org-1 Asset"))
	if err != nil {
		t.Fatalf("Upsert: %v", err)
	}

	_, err = db.GetKnowledgeAsset(ctx, saved.ID, 2)
...
func TestUpsertKnowledgeAsset_RejectsForeignSourceID(t *testing.T) {
	...
	_, err := db.UpsertKnowledgeAsset(ctx, hostile)
	if err == nil {
...
func TestSetKnowledgeAssetState_ForeignOrgIsIgnored(t *testing.T) {
	...
	sid := mustSetupSource(t, db, 1)
	saved, err := db.UpsertKnowledgeAsset(ctx, newTestAsset(1, sid, "ext_1", "Org-1"))
	if err != nil {
		t.Fatalf("Upsert: %v", err)
	}

	// Org-2 attempts to hide org-1's asset.
	if err := db.SetKnowledgeAssetState(ctx, saved.ID, 2, assets.StateHidden); err != sql.ErrNoRows {
		t.Errorf("foreign-org state change should return sql.ErrNoRows; got %v", err)
	}

	// Org-1's asset is unchanged.
	got, err := db.GetKnowledgeAsset(ctx, saved.ID, 1)
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
...
func TestUpsertKnowledgeAsset_IdempotentReSync(t *testing.T) {
	...
	first, err := db.UpsertKnowledgeAsset(ctx, newTestAsset(1, sid, "stable_gid_42", "Original Title"))
	if err != nil {
	...
	got, err := db.UpsertKnowledgeAsset(ctx, second)
	if err != nil {
	...
	list, err := db.ListKnowledgeAssetsForOrg(ctx, 1, assets.ListFilter{SourceID: sid})
	if err != nil {
		t.Fatalf("List: %v", err)
	}
	if len(list) != 1 {
...
func TestUpsertKnowledgeAsset_OperatorStateSurvivesReSync(t *testing.T) {
	...
	original, err := db.UpsertKnowledgeAsset(ctx, newTestAsset(1, sid, "stable_001", "v1"))
	if err != nil {
		t.Fatalf("first ingest: %v", err)
	}

	// Operator approves, pins, and boosts.
	if err := db.SetKnowledgeAssetState(ctx, original.ID, 1, assets.StateApproved); err != nil {
		t.Fatalf("SetState: %v", err)
	}
	if err := db.SetKnowledgeAssetPinned(ctx, original.ID, 1, true); err != nil {
		t.Fatalf("SetPinned: %v", err)
	}
	if err := db.SetKnowledgeAssetBoost(ctx, original.ID, 1, 75); err != nil {
	...
	resync := newTestAsset(1, sid, "stable_001", "v2 — updated title")
	resync.Description = "new description from ingestor"
	if _, err := db.UpsertKnowledgeAsset(ctx, resync); err != nil {
		t.Fatalf("re-ingest: %v", err)
	}

	got, err := db.GetKnowledgeAsset(ctx, original.ID, 1)
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	// Ingestor-controlled — updated.
	if got.Title != "v2 — updated title" {
...
func TestUpsertKnowledgeAsset_MetricsSurviveReSync(t *testing.T) {
	...
	a, err := db.UpsertKnowledgeAsset(ctx, newTestAsset(1, sid, "stable_m", "with metrics"))
	if err != nil {
		t.Fatalf("Upsert: %v", err)
	}
	...
	for range 3 {
		if err := db.IncrementKnowledgeAssetRetrieval(ctx, a.ID, 1); err != nil {
			t.Fatalf("Increment: %v", err)
		}
	}

	// Re-ingest.
	if _, err := db.UpsertKnowledgeAsset(ctx, newTestAsset(1, sid, "stable_m", "with metrics v2")); err != nil {
		t.Fatalf("re-ingest: %v", err)
	}

	got, err := db.GetKnowledgeAsset(ctx, a.ID, 1)
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
...
func TestListKnowledgeAssetsForOrg_StateFilter(t *testing.T) {
	...
	all, err := db.ListKnowledgeAssetsForOrg(ctx, 1, assets.ListFilter{})
	if err != nil {
	...
	// Approved-only — what the retrieval engine reads.
	approved, err := db.ListKnowledgeAssetsForOrg(ctx, 1, assets.ListFilter{
...
// transaction. Asserts the count too — operators need to know the
// blast radius.
func TestDeleteKnowledgeSourceForOrg_CascadesAssets(t *testing.T) {
	...
	for i := range 4 {
		if _, err := db.UpsertKnowledgeAsset(ctx, newTestAsset(1, sid, "ext_"+itoa(i), "row "+itoa(i))); err != nil {
			t.Fatalf("seed: %v", err)
		}
	}

	deleted, err := db.DeleteKnowledgeSourceForOrg(ctx, sid, 1)
	if err != nil {
	...
	left, err := db.ListKnowledgeAssetsForOrg(ctx, 1, assets.ListFilter{})
	if err != nil {
...
func TestSetKnowledgeAssetBoost_Clamps(t *testing.T) {
	...
	ctx := context.Background()
	sid := mustSetupSource(t, db, 1)
	a, err := db.UpsertKnowledgeAsset(ctx, newTestAsset(1, sid, "ext_b", "boost test"))
	if err != nil {
		t.Fatalf("Upsert: %v", err)
	}
	...
	for _, c := range cases {
		if err := db.SetKnowledgeAssetBoost(ctx, a.ID, 1, c.in); err != nil {
			t.Fatalf("SetBoost(%d): %v", c.in, err)
		}
		got, _ := db.GetKnowledgeAsset(ctx, a.ID, 1)
		if got.Boost != c.want {
...
func TestListKnowledgeAssetsForOrg_SearchFilter(t *testing.T) {
	...
	hits, err := db.ListKnowledgeAssetsForOrg(ctx, 1, assets.ListFilter{SearchQ: "cat"})
	if err != nil {
