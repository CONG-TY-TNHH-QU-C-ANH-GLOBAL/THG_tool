...
//
// SQLite version requirement: 3.35+ for RETURNING support.
// modernc.org/sqlite embeds a recent SQLite (currently 3.46.x).
// Verified at boot indirectly — any failing RETURNING query surfaces
// as a clear error at the first INSERT, not a silent fallback.
type SQLiteDialect struct{}

// NewSQLiteDialect returns the singleton SQLite dialect. The dialect
// is stateless so a zero-value SQLiteDialect{} is equally valid; this
// constructor exists to keep call sites readable (`dbutil.NewSQLiteDialect()`
// reads as "I want SQLite").
...
// InsertReturningID for SQLite uses QueryRowContext + Scan against the
// RETURNING clause. We could also use ExecContext + LastInsertId on
// SQLite specifically; using RETURNING keeps the call site dialect-
