...
type Config struct {
	...
	// Reel generation (services/reel). RenderProvider selects the VideoRenderer adapter
	// ("fake" = zero-cost dev/CI default, "cloudflare" = real/paid). ReelWebhookSecret
	// validates inbound render webhooks (HMAC-SHA256). Cloudflare creds are unused until
	// the real adapter lands.
	RenderProvider       string
	ReelWebhookSecret    string
	OpenAIReelScriptModel string
	CloudflareAPIToken   string
	CloudflareAccountID  string
	// ReelFakeAutocomplete (DEV ONLY): when RenderProvider=fake, the renderer self-posts a
	// signed done-webhook per shot so the full flow completes in the UI without a real
	// provider. Default OFF — never enabled in tests/production.
	ReelFakeAutocomplete bool

	// Real render pipeline (RenderProvider=real): FAL text-to-video + HeyGen avatar + FPT.AI
	// TTS, stitched locally with ffmpeg. Keys are read from env only; never committed. The
	// finished mp4 is stored under ReelMediaDir and streamed via GET /api/reels/:id/video.
	FALKey         string
	...
	ReelMediaDir   string
	FFmpegPath     string
	// ReelLogoPath: brand watermark (e.g. thg-brand-icon.png) overlaid top-left on every
	// assembled reel. Empty → no watermark (honest degrade). Read from env only.
	ReelLogoPath string

	// Anthropic (Claude) — reel script engine. When AnthropicAPIKey is set the reel script
	// is generated by Claude (real AI) instead of the OpenAI/deterministic fallback. Key is
	// read from env only; never committed.
	AnthropicAPIKey      string
	AnthropicScriptModel string
	// ReelMarketingGuidePath: path to a marketing skill/notes file (tone, voice, brand
	// vocabulary, hashtags). When set, its content grounds the reel script prompt so AI copy
	// matches the company's marketing playbook. Empty → no extra grounding.
...
func Load() *Config {
	cfg := &Config{
		...
		RenderProvider:             getEnv("RENDER_PROVIDER", "fake"),
		ReelWebhookSecret:          getEnv("REEL_WEBHOOK_HMAC_SECRET", ""),
		OpenAIReelScriptModel:      getEnv("OPENAI_REEL_SCRIPT_MODEL", getEnv("OPENAI_COMMENT_MODEL", "gpt-4.1")),
		CloudflareAPIToken:         getEnv("CLOUDFLARE_API_TOKEN", ""),
		CloudflareAccountID:        getEnv("CLOUDFLARE_ACCOUNT_ID", ""),
		ReelFakeAutocomplete:       getEnvBool("REEL_FAKE_AUTOCOMPLETE", false),
		FALKey:                     getEnv("FAL_KEY", ""),
		FALVideoModel:              getEnv("FAL_VIDEO_MODEL", "fal-ai/ltx-video"),
		HeyGenKey:                  getEnv("HEYGEN_API_KEY", ""),
		HeyGenAvatarID:             getEnv("HEYGEN_AVATAR_ID", ""),
		HeyGenVoiceID:              getEnv("HEYGEN_VOICE_ID", ""),
		FPTTTSKey:                  getEnv("FPT_TTS_KEY", ""),
		FPTVoice:                   getEnv("FPT_VOICE", "banmai"),
		ReelMediaDir:               getEnv("REEL_MEDIA_DIR", "data/reel-media"),
		FFmpegPath:                 getEnv("FFMPEG_PATH", "ffmpeg"),
		ReelLogoPath:               getEnv("REEL_LOGO_PATH", ""),
		AnthropicAPIKey:            getEnv("ANTHROPIC_API_KEY", ""),
		AnthropicScriptModel:       getEnv("ANTHROPIC_SCRIPT_MODEL", "claude-sonnet-4-6"),
		ReelMarketingGuidePath:     getEnv("REEL_MARKETING_GUIDE", ""),
