...
**FIND_CUSTOMERS/FIND_CANDIDATES → scrape_all**
Triggers: "cào tất cả", "quét hết", "scan all", "quét hết rồi inbox giúp tôi"
...
"quét hết rồi inbox giúp tôi" → scrape_all + inbox_all_leads (sau khi scrape xong)
...
// 10 tools: scrape, manage, query, engage, AND configure
...
var agentTools = []map[string]any{
	...
	},
	{
		...
		"function": map[string]any{
			"name":        "scrape_all",
			"description": "Cào tất cả groups đang active. Dùng khi user nói 'cào tất cả', 'quét hết', 'scan all'",
			"parameters": map[string]any{
				"type": "object",
				"properties": map[string]any{
					"platform": map[string]string{"type": "string", "description": "Platform: facebook, tiktok, hoặc zalo"},
				},
			},
		},
	},
