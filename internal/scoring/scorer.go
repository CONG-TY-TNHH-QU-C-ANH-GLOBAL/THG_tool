...
// Guidance carries org-specific phrasing that the scorer respects. Every field
// is optional; nothing here is hardcoded for a vertical. Without guidance the
// scorer is a generic relevance + engagement + quality scorer and never
// rejects on its own. Vertical reasoning belongs in the org's BusinessProfile
// (consumed by the AI classifier) and the brain-built market_signal_gate.
...
// Score computes a 0–100 lead score from generic content signals.
...
// ScoreWithGuidance applies generic relevance + engagement + quality scoring
// and respects the optional org-driven Guidance. The only path to
// Category="rejected" is an explicit phrase match against
// Guidance.RejectPhrases (sourced from the org's negative_signals /
// reject_rules). The scorer itself owns no industry-specific vocabulary —
// CLAUDE.md hard rule: business profile drives classification; do not hardcode
// one industry into the scorer.
func (s *Scorer) ScoreWithGuidance(content string, keywords []string, reactions, comments int, authorURL string, guidance Guidance) Result {
	...
	if hit := firstMatch(lower, normalizePhrases(guidance.RejectPhrases)); hit != "" {
		return Result{
			Score:    0,
			Category: "rejected",
			Signals:  []string{"reject:org_negative_signal:" + hit},
		}
	}
	...
	if hit := firstMatch(lower, normalizePhrases(guidance.TargetSignals)); hit != "" {
		score += 25
		signals = append(signals, "target_signal:"+hit)
	}
...
func firstMatch(content string, phrases []string) string {
