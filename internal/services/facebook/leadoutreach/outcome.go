...
// queueMessage inserts one queued outbound message and records the queue-stage
// outcome via the OutboundRecorder port (ARCHCM2c seam — no direct *store.Store).
// Returns a non-nil error only on a hard store failure (preserving the original
// `return "", err`); a policy denial is recorded as a skip.
func (c *Context) queueMessage(ctx context.Context, lead models.Lead, targetURL, content, retrievalID string, st *State) error {
...
// Mode derives the queue-result mode label from how many messages were
...
func Mode(approvedCount, queued int, requestedAuto bool) string {
	switch {
...
func outreachRiskDetails(st *State, accountID int64) string {
	if st.riskBlockSeen {
...
// FormatResult emits the run notification + structured log line and returns
// the caller-facing summary (comment vs other msgType), preserving the original
// strings exactly.
func (c *Context) FormatResult(ctx context.Context, requestedAuto bool, notify func(string), st *State) string {
	mode := Mode(st.approvedCount, st.Queued, requestedAuto)
...
func (c *Context) formatCommentResult(ctx context.Context, st *State, errDetails string) string {
