...
func (s *Server) signupUser(c *fiber.Ctx) error {
	...
	user := &models.User{
		OrgID:        0, // no org yet — onboarding will assign one
		Email:        req.Email,
		Name:         req.Name,
		PasswordHash: hash,
		Role:         models.RoleAdmin, // will become admin of their future org
	}
	...
	accessToken, _ := authpkg.GenerateAccessToken(userID, 0, req.Email, string(models.RoleAdmin), s.cfg.JWTSecret)
