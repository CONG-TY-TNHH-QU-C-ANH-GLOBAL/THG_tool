...
func (h *Handler) googleLoginRedirect(c *fiber.Ctx) error {
	if h.deps.GoogleClientID == "" {
		return c.Status(501).JSON(fiber.Map{"error": "Google OAuth not configured â€” set GOOGLE_CLIENT_ID"})
	}
	...
	q := url.Values{
		"client_id":     {h.deps.GoogleClientID},
		"redirect_uri":  {h.deps.GoogleRedirectURI},
		"response_type": {"code"},
...
func (h *Handler) googleCallback(c *fiber.Ctx) error {
	...
	accessToken, err := exchangeGoogleCode(code, h.deps.GoogleClientID, h.deps.GoogleClientSecret, h.deps.GoogleRedirectURI)
	if err != nil {
	...
	// Find or create user â€” Google Sign-In creates new accounts on first login.
	isNew := false
	user, err := h.deps.DB.GetUserByEmail(info.Email)
	if err != nil || user == nil {
		orgID := int64(0)
		role := models.RoleAdmin
		var provisionedClaim *store.ProvisionedOrgClaim
		if claim, claimErr := h.deps.DB.FindProvisionedOrgByEmail(info.Email); claimErr != nil {
		...
		newID, createErr := h.deps.DB.CreateUser(&models.User{
			OrgID:        orgID,
			Email:        info.Email,
			Name:         info.Name,
			PasswordHash: "", // no password â€” Google-only account
			Role:         role,
		})
		if createErr != nil {
		...
		if claimErr := h.completeProvisionedClaim(newID, provisionedClaim, c.IP()); claimErr != nil {
			log.Printf("[GoogleAuth] Complete provisioned claim failed: %v", claimErr)
			return redirectWithError(c, "workspace assignment failed")
		}
		user, err = h.deps.DB.GetUserByID(newID)
		if err != nil || user == nil {
			return redirectWithError(c, "account lookup failed")
		}
		isNew = true
		log.Printf("[GoogleAuth] Auto-created user: %s (id=%d)", info.Email, newID)
	} else if _, err := h.attachProvisionedOrgIfNeeded(user, c.IP()); err != nil {
		log.Printf("[GoogleAuth] Provisioned org claim failed for %s: %v", user.Email, err)
		return redirectWithError(c, "workspace assignment failed")
	}

	// Issue JWT pair
	jwtToken, err := authpkg.GenerateAccessToken(user.ID, user.OrgID, user.Email, string(user.Role), h.deps.JWTSecret)
	if err != nil {
	...
	// cookie server-side, so the SPA never sees the JWT â€” earlier
	...
	c.Cookie(&fiber.Cookie{
		Name:     "g_at",
		Value:    jwtToken,
		Path:     cookiePath, // /api/auth â€” only sent to the exchange endpoint
		MaxAge:   60,         // 60s â€” server reads and clears it immediately
		HTTPOnly: true,
	...
	h.deps.DB.InsertAuditLog(user.ID, action, c.IP(), fmt.Sprintf(`{"email":%q}`, user.Email))
...
func (h *Handler) googleToken(c *fiber.Ctx) error {
	token := c.Cookies("g_at")
	if token == "" {
		return c.Status(400).JSON(fiber.Map{"error": "no pending Google auth"})
	}
	// Clear the cookie â€” match Path/HTTPOnly/SameSite so the browser
	...
	claims, err := authpkg.ValidateAccessToken(token, h.deps.JWTSecret)
	if err != nil {
		return c.Status(401).JSON(fiber.Map{"error": "invalid token"})
	}

	user, err := h.deps.DB.GetUserByEmail(claims.Email)
	if err != nil || user == nil {
	...
	if claimed, err := h.attachProvisionedOrgIfNeeded(user, c.IP()); err != nil {
		...
		token, err = authpkg.GenerateAccessToken(user.ID, user.OrgID, user.Email, string(user.Role), h.deps.JWTSecret)
		if err != nil {
