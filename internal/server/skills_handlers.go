...
func (s *Server) skillsAll(c *fiber.Ctx) error {
	if s.agent == nil {
		return c.Status(503).JSON(fiber.Map{"error": "ai agent not configured"})
	}
	registry := s.agent.SkillRegistry()
...
func (s *Server) skillSetEnabled(c *fiber.Ctx, enabled bool) error {
	if s.agent == nil || s.agent.SkillRegistry() == nil {
		return c.Status(503).JSON(fiber.Map{"error": "skill registry not initialised"})
	}
	...
	if s.agent.SkillRegistry().Get(skillID) == nil {
	...
	if err := s.db.SetOrgSkillEnabled(c.Context(), orgID, skillID, enabled, userID); err != nil {
		return c.Status(500).JSON(fiber.Map{"error": err.Error()})
	}
	return c.JSON(fiber.Map{"id": skillID, "enabled": enabled})
}

func (s *Server) skillEnable(c *fiber.Ctx) error  { return s.skillSetEnabled(c, true) }
func (s *Server) skillDisable(c *fiber.Ctx) error { return s.skillSetEnabled(c, false) }
...
func (s *Server) skillExecutions(c *fiber.Ctx) error {
	orgID, _ := c.Locals("org_id").(int64)
	if orgID <= 0 {
		return c.Status(400).JSON(fiber.Map{"error": "org context required"})
	}
	...
	rows, err := s.db.ListRecentSkillExecutions(c.Context(), orgID, limit)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{"error": err.Error()})
	}
