...
func (s *Server) listDataSources(c *fiber.Ctx) error {
	orgID := c.Locals("org_id").(int64)
	sources, err := s.db.ListDataSources(orgID)
	if err != nil {
...
func (s *Server) createDataSource(c *fiber.Ctx) error {
	orgID := c.Locals("org_id").(int64)
	var req struct {
	...
	id, err := s.db.CreateDataSource(&store.DataSource{
		OrgID:        orgID,
		Type:         req.Type,
		Name:         req.Name,
	...
	src, _ := s.db.GetDataSourceForOrg(orgID, id)
	return c.Status(201).JSON(src)
}

func (s *Server) syncDataSource(c *fiber.Ctx) error {
	orgID := c.Locals("org_id").(int64)
	id, err := strconv.ParseInt(c.Params("id"), 10, 64)
	if err != nil {
		return c.Status(400).JSON(fiber.Map{"error": "invalid id"})
	}
	src, err := s.db.GetDataSourceForOrg(orgID, id)
	if err != nil || src == nil {
	...
	switch src.Type {
	case "google_sheet":
		if err := s.syncGoogleSheetSource(c.UserContext(), orgID, src); err != nil {
			_ = s.db.UpdateDataSourceSyncResult(orgID, src.ID, "error", src.ItemCount, src.Summary, src.MetadataJSON, err.Error(), false)
	...
	case "google_drive":
		...
		if err := s.db.UpdateDataSourceSyncResult(orgID, src.ID, "needs_auth", 0, "", meta, msg, false); err != nil {
			return c.Status(500).JSON(fiber.Map{"error": err.Error()})
		}
	default:
	...
	_ = s.refreshDataSourcesContext(orgID)
	updated, err := s.db.GetDataSourceForOrg(orgID, id)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{"error": err.Error()})
	}
...
func (s *Server) deleteDataSource(c *fiber.Ctx) error {
	orgID := c.Locals("org_id").(int64)
	id, err := strconv.ParseInt(c.Params("id"), 10, 64)
	if err != nil {
		return c.Status(400).JSON(fiber.Map{"error": "invalid id"})
	}
	if err := s.db.DeleteDataSourceForOrg(orgID, id); err != nil {
		return c.Status(500).JSON(fiber.Map{"error": err.Error()})
	}
	_ = s.refreshDataSourcesContext(orgID)
...
func (s *Server) syncGoogleSheetSource(ctx context.Context, orgID int64, src *store.DataSource) error {
	...
	if err := s.db.UpdateDataSourceSyncResult(orgID, src.ID, "synced", itemCount, summary, string(metaBytes), "", true); err != nil {
		return err
	}
	return nil
}
...
func (s *Server) refreshDataSourcesContext(orgID int64) error {
	sources, err := s.db.ListDataSources(orgID)
	if err != nil {
		return err
	}
	...
	return s.db.SetContext(orgContextKey(orgID, "data_sources_summary"), strings.TrimSpace(b.String()))
}
