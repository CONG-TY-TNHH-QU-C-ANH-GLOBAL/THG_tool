...
**Status**: design (no code). Stage 3 §5 semantic cleanup deliverable per [RUNTIME_TOPOLOGY.md §4](RUNTIME_TOPOLOGY.md#4-append-only-boundaries).
...
- Migrating `execution_attempts` UPDATEs (the `Begin`→`AdvanceStatus`→`Finish` lifecycle UPDATEs). Those are intra-row state and the design intent — see [RUNTIME_TOPOLOGY.md §4](RUNTIME_TOPOLOGY.md#4-append-only-boundaries) "Append-only by design" table.
