...
Bug B — with real text now reaching execution, action #91 failed `c.rung2.no_terminal_from_content_script`. **Root cause (architectural, NOT FB blocking):** the rung2 path had the CONTENT SCRIPT navigate the tab to the permalink while holding the `onMessage` response channel open — any navigation reloads the document and destroys the awaiting content script, closing the channel before a terminal returns. **Fix:** route comments through Path 2 (`executeInGroupFeed`): the BACKGROUND navigates to `/groups/<g>/` first (crawler's proven `navigateAndVerify`), then the content script comments INLINE in the feed without navigating — its frame survives, terminal always returns. `outbox.js` routing switched rung2 → Path 2; manifest 0.5.14 → 0.5.15. `deliverCommentViaRung2`/`probeRung2`/`deliverCommentViaPermalink` now dead; deletion queued after Path 2 is verified. **Awaiting verification cycle** (watch `path2.*` notes).
