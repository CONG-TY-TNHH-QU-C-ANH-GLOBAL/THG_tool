...
Bug B — with real text now reaching execution, action #91 failed `c.rung2.no_terminal_from_content_script` (rung2 had the CONTENT SCRIPT navigate while holding the response channel → channel closed mid-flow). First attempt: route through Path 2 (`executeInGroupFeed`, comment inline in group feed). **Operator rejected it and telemetry proved it wrong**: action #92 → `path2.article_not_found_in_feed: scrolls=8 articles_in_dom=2 nav_at_entry=https://www.facebook.com/ landed_at=https://www.facebook.com/`. The nav did NOT reach the group — it landed on the HOME feed, then scrolled home looking for the post (found 2 articles, never the target). Scrolling a feed to rediscover a known post is the wrong model. **Current fix:** removed the comment-specific branches entirely; comments now fall through to the generic crawler-nav path — `navigateAndVerify(targetUrl)` opens a fresh tab directly on the POST PERMALINK (crawler's chrome.tabs.create + retry + settle), then `thg_execute_outbound` types on that loaded page without the content script navigating (no `no_terminal`, no feed scroll). manifest 0.5.15 → 0.5.16. rung2/probe/permalink/Path2 delivery functions now all dead; deletion queued after this path is verified. **Awaiting verification.** If telemetry again shows `landed_at=https://www.facebook.com/` (redirect of the direct permalink nav), DOM automation is fundamentally losing to FB's redirect and the GraphQL track is next.
