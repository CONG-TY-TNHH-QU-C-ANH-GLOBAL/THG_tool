...
**Why we ship the wave with these violations carried forward**: Phase 5B mandate from the user was "preserve byte-for-byte". Fixing the append-only invariant is a semantic change (flip from row-mutation to event-emission). The migration design landed as [append-only-ledger.md](../domains/facebook-sales-intelligence/features/outbound-actions/implementation/append-only-ledger.md) — 3-PR staged plan (additive schema → reader migration → writer cutover). Implementation is queued; the design itself unblocks Stage 3 §5 because the path forward is documented + reviewable.
