...
| **PR-M3** | Pure policy package (planned name `freshlead`; shipped as `internal/services/facebook/crawlcampaign`) — freshness gate, frontier, scheduler decision, reason codes — + store CRUD. Shipped in three slices: **M3A** domain types + pure freshness policy (`internal/services/facebook/crawlcampaign`); **M3B** PostgreSQL queue/lifecycle store — enqueue/claim/heartbeat/recover (`internal/store/crawlrun`, migration 0118 claim index); **M3C** atomic `ApplyRunResult` — one transaction applying the fenced terminal transition, counters, `facebook_crawl_lead_index` reservations (`lead_id` NULL until PR-M5), and the source `last_run_at`/cursor stamp, with real-PostgreSQL fencing/idempotency/rollback/concurrency tests. | No wiring; dead code with tests. |
