...
const BaseAPI = "https://sale.thgfulfill.com"
...
func main() {
	...
	var fbName string
	fmt.Print(">> Đặt tên cho tài khoản Facebook này (VD: Nick Chính Sale 1): ")
	fmt.Scanln(&fbName)
	...
	if err != nil {
		log.Fatalf("❌ Lỗi không mở được Chrome. Bạn có đang cài đặt Chrome trên máy này không? Lỗi: %v", err)
	}
...
WaitForLogin:
	for {
		...
		case <-timeoutCtx.Done():
			fmt.Println("❌ Hết thời gian đăng nhập (5 phút). Chương trình tự động kết thúc.")
			os.Exit(1)
		case <-ticker.C:
			// Đọc cookie
	...
	fmt.Println("Ấn phím Enter để đóng chương trình...")
